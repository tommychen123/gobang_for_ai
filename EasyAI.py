...
class EasyAI():
    ...
    def genmove(self, board, turn):  # »ñÈ¡ËùÓÐ¿ÕÎ»
        moves = []
        for y in range(self.len):
            for x in range(self.len):
                if board[y][x] == 0:
    ...
    def search(self, board, turn):  # ËÑË÷×î¼ÑÂä×ÓÎ»ÖÃ
    ...
    def getScore(self, mine_count, opponent_count):    # ¼ÆËã·ÖÊý
        mscore, oscore = 0, 0
    ...
    def evaluatePoint(self, board, x, y, mine, opponent):
        ...
        for i in range(4):
            if self.record[y][x][i] == 0:
                self.analysisLine(
                    board, x, y, i, dir_offset[i], mine, opponent, self.count[mine-1])
            else:
                self.save_count += 1
    ...
    def analysisLine(self, board, x, y, dir_index, dir, mine, opponent, count):
        ...
        # ºóÎªÆå¾Ö·ÖÎö£º·Ö±ðÕë¶ÔËÄÁ¬Öé¡¢ÈýÁ¬ÖéµÈÇé¿ö
        if m_range == 5:
            count[FIVE] += 1
